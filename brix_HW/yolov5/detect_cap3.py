...
# ===== 등급 모델 =====
sys.path.append(str(Path.home() / "brixproject/brix_model"))
from model2 import load_ensemble, predict_grade
                ...
                for x1, y1, x2, y2, tid in tracks:
                    ...
                    if save_crop_once and (tid not in saved_ids):
                        ...
                        if crop.size != 0 and save_img:
                            ...
                            # 저장 즉시 등급 추론
                            try:
                                grade = predict_grade(str(out_path))
                                print(f"GRADE:{grade}:{str(out_path)}", flush=True)
                            except Exception as e:
                                print(f"MODEL_ERROR:{str(out_path)}:{e}", flush=True)
